...
say "1/3 dry run through the pipe entry point"
start=$(date +%s)
...
say "3/3 re-run (idempotency: everything should skip)"
start=$(date +%s)
if ! sh "$ROOT/install.sh"; then
...
say "PASS (full: ${full_time}s, re-run: ${rerun_time}s)"
